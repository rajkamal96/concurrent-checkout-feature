...
| 5 | **Payment timeout** | 6 seconds (`Promise.race`) | No timeout / 30 s | Assignment says provider "sometimes never replies". Without a timeout, reserved stock would stay locked indefinitely. 6 s gives headroom below the 8-s provider SLA. **Caveat:** `Promise.race` does not cancel the underlying payment call — if the real provider replies *after* the timeout, the charge may succeed while stock was already returned. Production fix: cancel via `AbortController`, or reconcile via the payment provider's webhook. |
